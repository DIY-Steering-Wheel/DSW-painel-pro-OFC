...
def _runtime():
    ...
    if _runtime_module is None:
        runtime_path = Path(__file__).resolve().parent / "runtime" / "beamngudp.py"
        spec = importlib.util.spec_from_file_location("beamng_runtime", runtime_path)
...
def collect(settings: dict) -> dict:
    ...
    module = _runtime()
    if not _initialized:
      module.iniciada(ip="127.0.0.1", port=4444)
      _initialized = True
    data = module.get()
    ...
    return {
        ...
        "lights_parking": lights.get("shift_light", 0),
        "lights_low_beam": lights.get("full_beam", 0),
